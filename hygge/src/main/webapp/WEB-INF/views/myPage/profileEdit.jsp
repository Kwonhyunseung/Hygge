...
<body>
    <div class="container">
        <h2>프로필 수정</h2>
        <form action="${pageContext.request.contextPath}/myPage/profileEdit" method="post" enctype="multipart/form-data">
            <!-- 파일 입력 요소를 폼 내부로 이동 -->
            <div class="profile-img-wrapper">
				<div class="profile-img" onclick="document.getElementById('profileUpload').click()">
				    <c:choose>
				        <c:when test="${empty myPage.profile_img}">
				            <i class="bi bi-person"></i>
				        </c:when>
				        <c:otherwise>
				            <img id="profileImgPreview" src="${pageContext.request.contextPath}/uploads/profile/${myPage.profile_img}" alt="profile_img">
				        </c:otherwise>
				    </c:choose>
				</div>
                <input type="file" id="profileUpload" accept="image/*" name="profileImg_File" style="display: none;" onchange="previewProfileImage(event)">
            </div>
    ...
    checkboxes.forEach((checkbox, index) => {
        checkbox.addEventListener('change', () => {
            if (checkbox.checked) {
                ...
                categoryItems[index].style.backgroundColor = '#b2cc85';
                categoryItems[index].style.color = 'white';
            } else {
                selectedCategories--;
                categoryItems[index].style.backgroundColor = '#e0e0e0';
    ...
    function previewProfileImage(event) {
        ...
        reader.onloadend = function() {
            const imgElement = document.getElementById('profileImgPreview');
